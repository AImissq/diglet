...
class Server extends EventEmitter {
  ...
  _respondErrorNoTunnelConnected(proxyId, request, response) {
    ...
    this.emit('REQUEST_ERR', { error, request, response });
  }

  /**
   * @private
   */
  _respondErrorNotServicable(proxyId, request, response) {
  ...
  routeHttpRequest(proxyId, request, response) {
    ...
    const getSocketHandler = (proxySocket, addSocketBackToPool) => {
      ...
      proxySocket.on('timeout', () => {
        this._respondErrorNotServicable(proxyId, request, response);
        clientRequest.abort();
        proxySocket.destroy();
      });
