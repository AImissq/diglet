...
class Server {
   ...
   * @param {Number} [options.proxyMaxConnections=24] - Max tunnels for proxy
  ...
  constructor(options) {
    ...
    this._server = net.createServer(sock => this._handleTunnelClient(sock));

    setInterval(() => this._heartbeat(), 60000);
  }

  /**
   * Every minute, we perform a request to each of our proxied tunnels to
   * trigger a socket cleanup
   * @private
   */
  _heartbeat() {
    // TODO
