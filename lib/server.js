...
class Server {
  ...
  _listenForClusterEvents() {
    process.on('message', (msg, handle) => {
      switch (msg.method) {
        case 'REGISTER_PROXY':
          this._registerProxy({ id: msg.params.proxy, alias: msg.params.alias });
          break;
        case 'REGISTER_TUNNEL':
          this._registerTunnel({ id: msg.params.proxy }, handle);
          break;
        case 'UNREGISTER_TUNNEL':
          this._unregisterProxy({ id: msg.params.proxy }, handle);
          break;
        default:
          this._logger.warn(`unknown method ${msg.method}`);
  ...
  _handleTunnelClient(socket) {
    ...
    socket.once('data', message => {
      ...
      if (!proxy) {
        ...
        if (this._opts.isClustered) {
          process.send({ method: 'REGISTER_PROXY', params: { proxy: id } });
        }
      }
      ...
      if (this._opts.isClustered) {
        process.send({ method: 'REGISTER_TUNNEL', params: { proxy: id } }, socket);
      }
    });
  ...
  /**
   * Creates a new proxy reference with the given ID
   */
  registerProxy({ id, alias }) {
    let proxy = this._proxies.get(id) || new Proxy({
      id,
      logger: this._logger,
      maxConnections: this._opts.proxyMaxConnections
    });
  ...
  /**
   * Registers a tunnel connection with the given proxy
   */
  registerTunnel({ id }, socket) {
  ...
  /**
   * Pops the socket from the proxy pool
   */
  lockTunnel({ id }, socket) {
    if (!this._proxies.has(id)) {
      return;
    }
    ...
    for (let t = 0; t < proxy._connectedSockets.length; t++) {
      if (proxy._connectedSockets[t] !== socket) {
        continue;
      }

      proxy._connectedSockets.splice(t, 1);
    }
  }
   ...
   * @private
   */
  _respondErrorNoTunnelConnected(proxyId, request, response) {
    ...
    response.end(JSON.stringify({
      message: 'Unable to route to tunnel, client is not connected'
    }));
  ...
  routeHttpRequest(proxyId, request, response) {
    ...
    const _sendCannotService = () => {
      ...
      response.setHeader('Content-Type', 'application/json');
      response.end(JSON.stringify({
        message: 'Client cannot service request at this time'
      }));
      request.connection.destroy();
    };
    ...
    const getSocketHandler = (proxySocket, addSocketBackToPool) => {
      if (this._opts.isClustered && proxySocket) {
        process.send({
          method: 'UNREGISTER_TUNNEL',
      ...
      if (responseDidFinish) {
        this._logger.warn('response already finished, aborting');
         ...
         if (addSocketBackToPool && this._opts.isClustered) {
          process.send({
            method: 'REGISTER_TUNNEL',
            params: { proxy: proxy.id }
          }, proxySocket);
        }
  ...
  _getRandomUnusedAlias() {
    ...
    if (this._aliases.has(alias)) {
      return Server._getRandomUnusedAlias(exclude, words);
    }
