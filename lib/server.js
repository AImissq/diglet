...
class Server {
  ...
  _handleTunnelClient(socket) {
    ...
    socket.once('data', message => {
      ...
      if (!proxy) {
        ...
        if (this._opts.isClustered) {
          process.send({ method: 'REGISTERED_PROXY', params: { proxy: id } });
        }
      }
  ...
  routeHttpRequest(proxyId, request, response) {
    ...
    if (!proxy) {
      if (this._opts.isClustered) {
        process.send({
          method: 'UNSERVICABLE_HTTPS',
          params: { proxy: proxyId, request, response }
        });
      } else {
  ...
  routeWebSocketConnection(proxyId, request, socket) {
    ...
    if (!proxy) {
      if (this._opts.isClustered) {
        process.send({
          method: 'UNSERVICABLE_WSS',
          params: { proxy: proxyId, request, socket }
        });
      } else {
        socket.destroy();
      }
