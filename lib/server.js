...
class Server {
   ...
   * @param {Number} [options.proxySocketTimeout=2000] - Max time to wait
   * before assuming a socket is dead
   * @param {Object} [options.logger=console] - Custom logger to use
   */
  constructor(options = {}) {
    options.proxySocketTimeout = options.proxySocketTimeout || 2000;
  ...
  routeHttpRequest(proxyId, request, response, callback) {
    ...
    function getSocketHandler(proxySocket, addSocketBackToPool) {
      ...
      function _setSocketTimeout(socket) {
        socket.setTimeout(self._opts.proxySocketTimeout);
        socket.on('timeout', () => clientRequest.abort());
      }

      self._logger.info('tunneling request through to client');
      response.once('finish', () => addSocketBackToPool());
      clientRequest.on('socket', (socket) => _setSocketTimeout(socket));
    ...
    self._logger.info('getting proxy tunnel socket back to client...');
