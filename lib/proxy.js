...
class Proxy extends EventEmitter {
  ...
  pop(socketHandler) {
    ...
    this._logger.debug('getting socket from proxy tunnel');
    ...
    if (!socket) {
      ...
      return this.getSocket(socketHandler);
    }

    this._logger.debug('got tunnel socket, passing to handler');
    socketHandler(socket, () => {
      this._logger.debug('socket handler finished, adding back to pool');
   ...
   */
  clean() {
    this._logger.debug('cleaning connection pool');
    this._waitingHandlers.forEach((handler) => handler(null));
    this._connectedSockets.forEach((socket) => socket.destroy());
    this.emit('end');
  }
   ...
   */
  push(socket) {
    this._logger.debug('handling incoming tunnel connection');
    ...
    this._logger.debug('establishing tunnel connection to client');
