...
class Proxy extends EventEmitter {
  ...
  getSocket(socketHandler) {
    ...
    self._logger.info('getting socket from proxy tunnel');

    if (!socket) {
      self._logger.warn('no socket available, queuing handler');
      return self._waitingHandlers.push(socketHandler);
    } else if (socket.destroyed) {
      self._logger.warn('get destroyed socket, getting another...');
      self._handleSocketClose(socket);
      return self.getSocket(socketHandler);
    }

    self._logger.info('got tunnel socket, passing to handler');
    socketHandler(socket, () => {
      self._logger.info('socket handler finished, adding back to pool');
  ...
  _cleanConnections() {
    ...
    self._logger.info('cleaning connection pool');
  ...
  _handleConnection(socket) {
    ...
    self._logger.info('handling incoming tunnel connection');
    ...
    self._logger.info('establishing tunnel connection to client');
    clearTimeout(self._connectionTimeout);
  ...
  _handleSocketClose(socket) {
    ...
    self._logger.info('tunnel socket closed');
  ...
  _destroy() {
    ...
    self._logger.info('destroying tunnel proxy');
