...
class Cluster extends EventEmitter {
  ...
  _routeIncomingHttps(proxy, request, response) {
    if (!this._proxies.has(proxy)) {
      if (cluster.isMaster) {
        return this.server._respondErrorNoTunnelConnected(proxy, request,
          response);
      } else {
        process.send({
   ...
   * @private
   */
  _registerProxy(params, worker) {
    this._proxies.set(params.proxy, worker);
  }
