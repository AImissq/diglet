...
class Tunnel extends EventEmitter {
  ...
  open() {
    const self = this;
    const tunnels = self._opts.maxConnections - self._tunnelsOpened;

    self._logger.info(
      'establishing tunnel to %s:%s',
    ...
    for (let i = 0; i < tunnels; i++) {
      self._logger.info('creating remote connection %s', i + 1);
  ...
  _handleTunnelOpen(tunnelConnection) {
    ...
    self._tunnelsOpened++;
    self._logger.info('a tunnel connection was opened');
    ...
    function _handleTunnelClose() {
      self._logger.warn('a tunnel connection was closed');
      self._tunnelsOpened--;
  ...
  _createRemoteConnection() {
    ...
    remoteConnection.once('connect', () => {
      self._logger.info('remote connection established');
  ...
  _createLocalConnection(remoteConnection) {
    ...
    self._logger.info('creating local connection...');
    ...
    remoteConnection.once('close', () => {
      self._logger.warn('remote connection closed, ending local connection');
      localConnection.end()
    });
  ...
  _handleLocalOpen(localConnection, remoteConnection) {
    ...
    self._logger.info('local connection opened');

    if (self._opts.localAddress !== 'localhost') {
      incomingStream = remoteConnection.pipe(self._transformHeaders());
    }

    if (self._opts.transform) {
      incomingStream = incomingStream.pipe(self._opts.transform);
    }

    self._logger.info('connecting local and remote connections');
  ...
  _handleRemoteError(remoteConnection, error) {
    ...
    self._logger.error('remote connection encountered error: %s', err.message);
