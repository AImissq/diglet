...
class Tunnel extends EventEmitter {
  ...
  get url() {
    ...
    const id = crypto.creatHash('rmd160')
      .update(pubkey)
      .digest('hex');

    return `http://${id}.${this._opts.remoteAddress}:${this._opts.remotePort}`;
  }
  ...
  open() {
    ...
    this.once('open', () => this.emit('established'));
    this.on('open', tunnel => this._handleTunnelOpen(tunnel));

    for (let i = 0; i < this._opts.maxConnections; i++) {
  ...
  _createLocalConnection(remoteConnection) {
    ...
    remoteConnection.once('close', () => {
      this._logger.debug('remote connection closed, ending local connection');
      localConnection.end();
      this._logger.debug('reopening remote tunnel connection');
      this._createRemoteConnection();
    });
  ...
  _handleLocalOpen(localConnection, remoteConnection) {
    ...
    stream = stream.pipe(this._opts.transform);
